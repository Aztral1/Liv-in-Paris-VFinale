...
class Program
{
    ...
    /// </summary>
    static string connectionString = "server=localhost;database=premierRenduPSI;user=root;password = Yannou01!;";
    ...
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            switch (choix)
            {
                ...
                case "5" :
                    double[,] matriceAdjacence = GenererMatriceAdjacence(connectionString);

                    // Appliquer l'algorithme Welsh-Powell
                    int[] resultColors = WelshPowell(matriceAdjacence);

                    // Afficher les couleurs des sommets
                    Console.WriteLine("Couleurs des sommets :");
                    for (int i = 0; i < resultColors.Length; i++)
                    {
                        Console.WriteLine($"Sommet {i} -> Couleur {resultColors[i]}");
                    }

                    // Afficher les sommets regroupés par couleur
                    Console.WriteLine("\nSommets par couleur :");
                    for (int couleur = 0; couleur <= resultColors.Max(); couleur++)
                    {
                        var sommets = new List<int>();
                        for (int i = 0; i < resultColors.Length; i++)
                        {
                            if (resultColors[i] == couleur)
                            {
                                sommets.Add(i);
                            }
                        }
                        Console.WriteLine($"Couleur {couleur} : {string.Join(", ", sommets)}");
                    }

                    // Vérifier si le graphe est biparti
                    bool biparti = EstBiparti(matriceAdjacence);
                    Console.WriteLine($"\nLe graphe est-il biparti ? {biparti}");

                    // Vérifier si le graphe est biplanaire
                    bool biplanaire = EstBiplanaire(matriceAdjacence);
                    Console.WriteLine($"Le graphe est-il biplanaire ? {biplanaire}");
                    break;
...
";
    ...
    public static int[] WelshPowell(double[,] mat_adj)
    {
        ...
        for (int count = 0; count < n; count++)
        {
            ...
            for (int v = 0; v < n; v++)
            {
                if (!dejavu[v] && degrees[v] > degreemax)
                {
                    degreemax = degrees[v];
                    sommetmax = v;
                }
            ...
            if (sommetmax != -1)
            {
                dejavu[sommetmax] = true;
                listSommet.Add(sommetmax);
            }
        }
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int j = i + 1; j < n; j++)
            {
                ...
                if (result[u] == -1 && !EstAdjacent(mat_adj, result, u, couleur))
                {
                    result[u] = couleur;
                }
            }
    ...
    public static bool EstBiplanaire(double[,] matriceAdjacence)
    {
        ...
        {
            for (int j = i + 1; j < taille; j++) // Parcours uniquement au-dessus de la diagonale
            {
    ...
    private static bool EstAdjacent(double[,] mat, int[] result, int sommet, int couleur)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (mat[sommet, i] != 0 && result[i] == couleur)
                return true;
        }
        return false;
    }
    ...
    {
        int taille = matriceAdjacence.GetLength(0);
        int[] couleurs = new int[taille]; // 0: non coloré, 1: couleur 1, -1: couleur 2

        Array.Fill(couleurs, 0);

        for (int i = 0; i < taille; i++)
        {
            if (couleurs[i] == 0)
            {
                if (!VerifierBipartiDFS(matriceAdjacence, i, 1, couleurs))
                {
                    return false;
                }
            }
    ...
    public static double[,] GenererMatriceAdjacence(string connectionString)
    {
        ...
        var relations = new List<(int, int)>(); // Liste des relations (idClient, idCuisinier)

        using (var connection = new MySqlConnection(connectionString))
        {
            connection.Open();

            // Récupérer tous les utilisateurs (clients et cuisiniers)
            using (var clientCommand = new MySqlCommand("SELECT idClient FROM client", connection))
            using (var reader = clientCommand.ExecuteReader())
            {
                while (reader.Read())
                {
                    int idClient = reader.GetInt32(0);
                    if (!utilisateurs.ContainsKey(idClient))
                    {
                        utilisateurs[idClient] = utilisateurs.Count;
                    }
                }
            }

            using (var cuisinierCommand = new MySqlCommand("SELECT idCuisinier FROM cuisinier", connection))
            using (var reader = cuisinierCommand.ExecuteReader())
            {
                while (reader.Read())
                {
                    int idCuisinier = reader.GetInt32(0);
                    if (!utilisateurs.ContainsKey(idCuisinier))
                    {
                        utilisateurs[idCuisinier] = utilisateurs.Count;
                    }
                }
            }

            // Récupérer les relations via les commandes
            using (var commandeCommand = new MySqlCommand("SELECT idClient, idCuisinier FROM commande", connection))
            using (var reader = commandeCommand.ExecuteReader())
            {
                while (reader.Read())
                {
                    int idClient = reader.GetInt32(0);
                    ...
                    if (utilisateurs.ContainsKey(idClient) && utilisateurs.ContainsKey(idCuisinier))
                    {
                        relations.Add((idClient, idCuisinier));
                    }
                }
            }
        }
        ...
        int taille = utilisateurs.Count;
        double[,] matriceAdjacence = new double[taille, taille];

        foreach (var (id1, id2) in relations)
        {
            int index1 = utilisateurs[id1];
            int index2 = utilisateurs[id2];

            matriceAdjacence[index1, index2] = 1; // Lien entre id1 et id2
            matriceAdjacence[index2, index1] = 1; // Lien réciproque
        }
    ...
    private static bool VerifierBipartiDFS(double[,] matrice, int sommet, int couleur, int[] couleurs)
    {
        ...
        for (int voisin = 0; voisin < matrice.GetLength(0); voisin++)
        {
            if (matrice[sommet, voisin] != 0)
            {
                ...
                if (couleurs[voisin] == 0 && !VerifierBipartiDFS(matrice, voisin, -couleur, couleurs))
                {
                    return false;
                }
            }
